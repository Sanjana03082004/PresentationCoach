...
2. The app encodes it in Base64 and sends it to Gemini’s endpoint:  
```

[https://generativelanguage.googleapis.com/v1beta/models/gemini-1.5-flash:generateContent?key=YOUR_API_KEY](https://generativelanguage.googleapis.com/v1beta/models/gemini-1.5-flash:generateContent?key=YOUR_API_KEY)
...
4. In your **app-level `build.gradle.kts`**, add:
...
1. Run the app on an emulator or physical Android device.
2. Tap **Select Video** → pick a `.mp4` file.
3. Wait while the progress bar shows “Analyzing…”.
4. Once analysis completes, read the AI feedback on-screen.
...
* If upload fails, check **Logcat** for messages like “Request JSON” or “API Error.”
* Ensure your device has Internet access; the API requires a live network.
...
📧 [sanjanamadpalwar@gmail.com](mailto:sanjanamadpalwar@gmail.com)
